...
ο οποίος πέτυχε 0.9495 validation accuracy και 0.1609 validation loss στο epoch 28, αριθμοί πολύ ανταγωνιστικοί σε σχέση με τον καλύτερο συνδυασμό υπερπαραμέτρων 
...
\begin{itemize}
    ...
    \item Data Augmentation στις κλάσεις με πολύ λιγότερα δείγματα με σκοπό την καταπολέμησης της ανισορροπί\-ας.
    \item Weighted Loss: Implement a weighted BCEWithLogitsLoss where classes with fewer positive samples (which are most of the diseases here) 
        are given a higher weight. This forces the model to pay more attention to correctly classifying these underrepresented positive cases.
    \item Threshold Optimization: The current model uses a fixed threshold of 0.5 for torch.sigmoid(outputs) > 0.5. Experiment with optimizing this 
        threshold per class on the validation set. A lower threshold might increase recall for positive cases.
\end{itemize}
