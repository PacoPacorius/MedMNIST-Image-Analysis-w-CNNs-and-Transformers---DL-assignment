...
μέρη. Στον κώδικα του πρώτου μέρους γίνεται πρώτα μια προετοιμασία του dataset, μετά ορίζεται
η αρχιτεκτονική του CNN και τέλος γίνεται το training, το validation το test και η παράθεση των αποτελεσμάτων. 
...
accuracy και loss για τα τριάντα epochs για κάθε hyperparameter που δοκιμάστηκε. Στην~\autoref{best_combination_plot} φαίνεται το 
γράφημα του καλύτερου hyperparameter combination ως παράδειγμα.

\section{Συμπεράσματα}
Dropout 0.2 κακό, 0.7 μέγιστο. Batchnorm καλύτερο από layernorm. Weight Decay $10^2$ κακό. Κανονικοποίηση δεδομένων εισόδου 
δεν έχει διαφορά και μερικές φορές έχει χειρότερη απόδοση.


\begin{itemize}
    \item CNN model αρχιτεκτονική -done
    \item Hyperparameter combinations. Combinations που δεν έτρεξαν και γιατί (RIP) -done
    \item Train, loop, validation functions. -done
    \item Main loop, αποθήκευση hyperparameter combinations και επιδόσεων. -done
    \item Plotting training and validation history. Σύγκριση-εύρεση καλύτερου hyperparameter combination, test dataset run. -done
    \item Συμπεράσματα για επίδοση hyperparameter combinations.
\end{itemize}

\section{Transfer Learning}
\subsection{Προετοιμασία του Dataset}
\subsection{Pre-Trained Μοντέλο}
\subsection{Training, Validation, Testing και Αποτελέσματα}
...
\begin{itemize}
    ...
    \item Επιλογή βέλτιστου μοντέλου με βάση το loss, όχι του accuracy, λόγω ανισορροπίας κλάσεων.
